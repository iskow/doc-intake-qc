...
- [ASSUMPTION: open] An 8B-class local instruct model clears the Phase 3 spot-check gate on this fixture. Untested — the first Phase 3 step is to measure it against the folder-name ground truth on ~10 docs *before* building the pipeline around it. If it flunks, escalate to a larger local model, or reopen the hosted-API question with evidence rather than a guess.
